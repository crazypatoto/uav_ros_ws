...
class UAVCollisionAvoidanceServerNode():
    def __init__(self):             
        ...
        rospy.Service('/uav0/uav_collision_avoidance/select_action', UCASelectAction, self.uav0_handle_action_selection)
        rospy.Service('/uav1/uav_collision_avoidance/select_action', UCASelectAction, self.uav1_handle_action_selection)
        rospy.Service('/uav2/uav_collision_avoidance/select_action', UCASelectAction, self.uav2_handle_action_selection)
        rospy.Service('/uav3/uav_collision_avoidance/select_action', UCASelectAction, self.uav3_handle_action_selection)
        rospy.Service('/uav4/uav_collision_avoidance/select_action', UCASelectAction, self.uav4_handle_action_selection)
        rospy.Service('/uav5/uav_collision_avoidance/select_action', UCASelectAction, self.uav5_handle_action_selection)
        rospy.Service('/uav6/uav_collision_avoidance/select_action', UCASelectAction, self.uav6_handle_action_selection)
        rospy.Service('/uav7/uav_collision_avoidance/select_action', UCASelectAction, self.uav7_handle_action_selection)
        rospy.Service('/uav8/uav_collision_avoidance/select_action', UCASelectAction, self.uav8_handle_action_selection)
        rospy.Service('/uav9/uav_collision_avoidance/select_action', UCASelectAction, self.uav9_handle_action_selection)
        rospy.Service('/uav_collision_avoidance/uavs_in_range', UAVsInRange, self.handle_uavs_in_range)        

        rospack = rospkg.RosPack()                
        model_path = rospack.get_path('uav_collision_avoidance')        
        model_path = os.path.join(model_path, 'weights')        

        self.agents = []
        for i in range(10):
            agent = SAC(10, 2, use_cpu=i%2) # 10 observations, 2 actions
            agent.load_checkpoint(model_path, evaluate=True)
            self.agents.append(agent)                 
    

    def uav0_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        ...
        if ns not in self.uav_namespaces:
            ...
            action = self.agents[0].select_action(observation, evaluate=True)            
    ...
    def uav1_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[1].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav2_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[2].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav3_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[3].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav4_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[4].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav5_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[5].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav6_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[6].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav7_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[7].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav8_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[8].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
    ...
    def uav9_handle_action_selection(self, req):
        ns = req.uavNamespace
        target = req.target
        target = np.array([target.x, target.y])
        ...
        if ns not in self.uav_namespaces:
            ...
            observation = self.get_observation(ns, target)                
            action = self.agents[9].select_action(observation, evaluate=True)            
            v = (action[0]/2+0.5) * MAX_SPEED
            theta = action[1] * math.pi
            converted_action = Vector3()
            converted_action.x = v*math.cos(theta)
